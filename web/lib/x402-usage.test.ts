...
describe("logX402Usage", () => {
  ...
  it("takes free-form params instead of a voting power for endpoints that aren't sized by one", () => {
    const spy = vi.spyOn(console, "log").mockImplementation(() => {});

    logX402Usage({ route: "v1/bribe-target", priceUsd: 0.1, refresh: false, params: { targetSharePct: 5 } });

    const logged = JSON.parse(spy.mock.calls[0][0] as string);
    expect(logged).toMatchObject({ route: "v1/bribe-target", priceUsd: 0.1, params: { targetSharePct: 5 } });
